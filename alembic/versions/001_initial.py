...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_athlete_competition_division", "athletes", ["competition_id", "division"]
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "order_in_competition", sa.Integer(), nullable=False, server_default="1"
        ),
        sa.Column("competition_id", sa.Integer(), nullable=False),
        sa.Column(
            "created_at", sa.DateTime(), nullable=False, server_default=sa.func.now()
    ...
    op.create_index(
        "ix_wod_competition_order", "wods", ["competition_id", "order_in_competition"]
    )
    ...
    op.create_index(
        "ix_audit_score_timestamp", "score_audit_logs", ["score_id", "timestamp"]
    )
    op.create_index(
        "ix_audit_user_timestamp", "score_audit_logs", ["user_id", "timestamp"]
    )
