...
from models import WOD, Competition, User
from scoring import format_result, get_competition_leaderboard
...
async def export_excel(
    ...
    for division, entries in sorted(leaderboards.items()):
        ...
        for row_idx, entry in enumerate(entries, 4):
            ...
            ws.cell(row=row_idx, column=2, value=entry.bib_number).alignment = (
                cell_alignment
            )
            ws.cell(row=row_idx, column=3, value=entry.athlete_name)
            ws.cell(row=row_idx, column=4, value=entry.box or "")
            ws.cell(row=row_idx, column=5, value=entry.total_points).alignment = (
                cell_alignment
            )

            # WOD scores
            wod_scores_map = {
                ws_data["wod_id"]: ws_data for ws_data in entry.wod_scores
            }
    ...
    filename = f"{competition.name.replace(' ', '_')}_resultados_{date_str}.xlsx"
...
async def export_csv(
    ...
    date_str = datetime.now().strftime("%Y%m%d")
    filename = f"{competition.name.replace(' ', '_')}_resultados_{date_str}.csv"
...
async def export_pdf(
    ...
    date_str = datetime.now().strftime("%Y%m%d")
    filename = f"{competition.name.replace(' ', '_')}_resultados_{date_str}.pdf"
